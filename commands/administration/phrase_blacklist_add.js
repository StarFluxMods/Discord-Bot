...
module.exports = {
	...
	async execute(interaction) {
        if (!await CommandUtils.EnsurePermissions(interaction, 'commands.phraseblacklistadd')) { return; }

        const phrase = interaction.options.getString('phrase').toLowerCase();

        const result = await CommandUtils.AddPhraseBlackList(phrase);
        if (result) {
            interaction.reply({ content: `Added \`${phrase}\` to the phrase blacklist.`, ephemeral: true });
        }
        else {
            interaction.reply({ content: `Updated \`${phrase}\` in the phrase blacklist.`, ephemeral: true });
        }
	},
};

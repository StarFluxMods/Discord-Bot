...
module.exports = {
	...
	async execute(interaction) {
        if (!await CommandUtils.EnsurePermissions(interaction, 'commands.phraseblacklistremove')) { return; }
        ...
        if (result) {
            interaction.reply({ content: `Removed \`${phrase}\` from the phrase blacklist.`, ephemeral: true });
        }
        else {
            interaction.reply({ content: `Could not find \`${phrase}\` in the phrase blacklist.`, ephemeral: true });
        }
	},
};

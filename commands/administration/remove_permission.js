...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setDescription('Removes a permission to a role')
        .addRoleOption(option => option.setName('role').setDescription('The role to remove the permission from').setRequired(true))
        .addStringOption(option => option.setName('permission').setDescription('The permission to from from the role').setRequired(true)),

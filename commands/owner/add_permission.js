...
module.exports = {
	...
	async execute(interaction) {
        if (!await CommandUtils.EnsurePermissions(interaction, 'commands.addpermission')) { return; }

        const role = interaction.options.getRole('role');
        const permission = interaction.options.getString('permission');
        const success = await PermissionManager.addPermission(role.id, permission);
        if (success) {
            await interaction.reply({ content: `Added permission \`${permission}\` to role \`${role.name}\``, ephemeral: true });
        }
        else {
            interaction.reply({ content: `Permission \`${permission}\` already exists for role \`${role.name}\``, ephemeral: true });
        }
	},
};

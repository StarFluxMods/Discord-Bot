...
module.exports = {
    ...
    async execute(interaction) {
        if (!(await CommandUtils.EnsurePermissions(interaction, 'commands.adduserpermission', true, true))) {
            return;
        }
        ...
        if (success) {
            await interaction.editReply({
                content: `Added permission \`${permission}\` to user \`${member.user.tag}\``,
                ephemeral: true,
            });
        } else {
            interaction.editReply({
                content: `Permission \`${permission}\` already exists for user \`${member.user.tag}\``,
                ephemeral: true,
            });
        }
    },

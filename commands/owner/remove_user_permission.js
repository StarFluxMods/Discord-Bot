...
module.exports = {
	...
	async execute(interaction) {
        if (!await CommandUtils.EnsurePermissions(interaction, 'commands.removeuserpermission')) { return; }
        ...
        if (success) {
            await interaction.reply({ content: `Removed permission \`${permission}\` from user \`${member.user.tag}\``, ephemeral: true });
        }
        else {
            interaction.reply({ content: `Permission \`${permission}\` does not exist for user \`${member.user.tag}\``, ephemeral: true });
        }
	},
};

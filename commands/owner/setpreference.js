...
module.exports = {
	...
	async execute(interaction) {
        if (!await CommandUtils.EnsurePermissions(interaction, 'commands.setpreference')) { return; }
        ...
        if (success) {
            await interaction.reply({ content: `Set preference \`${key}\` to \`${value}\``, ephemeral: true });
        }
        else {
            await interaction.reply({ content: `Updated preference \`${key}\` to \`${value}\``, ephemeral: true });
        }
	},
};

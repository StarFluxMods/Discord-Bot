...
module.exports = { addPermission, removePermission, getPermissions, getRoles, hasPermission, getRole, setupMasterPermission, hasPermissionLike, addPermissionToUser, removePermissionFromUser, getUserPermissions };
...
async function addPermissionToUser(user, permission) {
    ...
    const existingPermission = await SQLManager.UserPermissions.findOne({ where: { UserID: user, Permission: permission } });
    if (existingPermission) {
        return false;
    }
    await SQLManager.UserPermissions.create({ UserID: user, Permission: permission });
    return true;
}
...
async function getUserPermissions(member) {
    const permissions = await SQLManager.UserPermissions.findAll({ where: { UserID: member.id } });
...
async function hasPermission(member, permission) {
    ...
    const userPermissions = await getPermissions(member.id);
